...
This is where an overview of the server architecture is available.
...
In total there are six servers, two masters and four nodes, with each master controlling two nodes. Each server can be acessed by using the internal IP via ssh. The two public IP's for the project only has port 80 and port 443 open. Each node has been configured to use the ```10.14.0.0/16``` subnet for the local docker daemon. For the swarm overlay network, the ```10.10.0.0/16``` subnet is used.
